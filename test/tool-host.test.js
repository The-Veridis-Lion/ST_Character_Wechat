...
test("tool host rejects legacy timeline write CLI-shaped fields", async () => {
  ...
  await assert.rejects(async () => {
    await host.invokeTool("st_character_wechat_timeline_write", {
      date: "2026-04-21",
      events: [],
      eventsJson: "{\"events\":[]}",
    }, {});
...
test("tool host exposes structured timeline read tools", async () => {
  const host = createHost();
  const readResult = await host.invokeTool("st_character_wechat_timeline_read", {
    date: "2026-04-21",
  }, {});
  const categoriesResult = await host.invokeTool("st_character_wechat_timeline_categories", {}, {});
  const proposalsResult = await host.invokeTool("st_character_wechat_timeline_proposals", {
    date: "2026-04-21",
  }, {});
...
test("tool host validates structured reminder input types", async () => {
  const host = createHost();
  await assert.rejects(async () => {
    await host.invokeTool("st_character_wechat_reminder_create", {
      text: "ping me",
      delayMinutes: "30",
    }, {});
...
test("tool host accepts structured timeline screenshot input", async () => {
  const host = createHost();
  const result = await host.invokeTool("st_character_wechat_timeline_screenshot", {
    selector: "timeline",
    range: "day",
    date: "2026-04-21",
    width: 1440,
...
test("tool host descriptions include schema summary for models that only surface descriptions", () => {
  const host = createHost();
  const timelineWrite = host.listTools().find((tool) => tool.name === "st_character_wechat_timeline_write");
...
test("tool host exposes local whereabouts tools", async () => {
  const host = createHost();
