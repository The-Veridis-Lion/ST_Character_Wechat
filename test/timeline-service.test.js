...
const TMP_DIR = path.join(os.tmpdir(), "st-character-wechat-timeline-service-test");

function createService() {
  const calls = [];
  const service = new TimelineService({
    config: {
      stateDir: path.join(TMP_DIR, "state"),
      timelineScreenshotQueueFile: path.join(TMP_DIR, "timeline-screenshot-queue.json"),
    },
    timelineIntegration: {
...
test("timeline service serializes structured screenshot options", async () => {
  ...
  const result = await service.captureScreenshot({
    outputFile: path.join(TMP_DIR, "timeline-shot.png"),
    selector: "analytics",
    range: "day",
    date: "2026-04-21",
    category: "work",
  ...
  assert.equal(result.outputFile, path.join(TMP_DIR, "timeline-shot.png"));
  assert.deepEqual(calls, [
    {
      subcommand: "screenshot",
      args: [
        "--output", path.join(TMP_DIR, "timeline-shot.png"),
        "--selector", "analytics",
        "--range", "day",
        "--date", "2026-04-21",
        "--category", "work",
