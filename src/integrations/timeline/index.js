...
function resolveTimelineBinPath() {
  return path.resolve(__dirname, "..", "..", "..", "scripts", "local-timeline.js");
}
...
function buildTimelineSpawnSpec(binPath, args = [], nodePath = process.execPath) {
  return {
    command: nodePath,
    args: [binPath, ...args],
  };
...
function resolveTimelineChromePath() {
  const configured = normalizeText(process.env.TIMELINE_FOR_AGENT_CHROME_PATH)
    || normalizeText(process.env.ST_CHARACTER_WECHAT_SCREENSHOT_CHROME_PATH);
  if (configured) {
    return configured;
  }
