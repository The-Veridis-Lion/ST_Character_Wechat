...
function cleanupStaleAccountsForUserId(config, activeAccount) {
  ...
  for (const staleAccount of staleAccounts) {
    ...
    console.log(`[st-character-wechat] removed stale account ${staleAccount.accountId} for userId ${activeUserId}`);
  }
...
async function runLoginFlow(config) {
  console.log("[st-character-wechat] starting WeChat QR login...");
