...
function createClaudeCodeRuntimeAdapter(config) {
  ...
  const ipcSocketPath = path.join(
    config.stateDir || path.join(os.homedir(), ".st-character-wechat"),
  ...
  function ensureClient(workspaceRoot) {
    ...
    const projectSettings = ensureClaudeProjectMcpConfig({
      workspaceRoot,
      projectHome: process.env.ST_CHARACTER_WECHAT_HOME || path.resolve(__dirname, "..", "..", "..", ".."),
    });
    console.log(
  ...
  return {
    ...
    async sendTextTurn({ bindingKey, workspaceRoot, text, metadata = {}, model = "" }) {
      ...
      const useSharedOpeningInstructions = !metadata?.characterChat;
      let attached;
      try {
      ...
      const outboundText = openingTurn && useSharedOpeningInstructions ? buildOpeningTurnText(config, text) : text;
