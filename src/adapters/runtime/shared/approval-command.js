...
function canonicalizeCommandTokens(tokens) {
  ...
  if (executable === "node" || executable === "node.exe") {
    ...
    if ((binPath === "./bin/st-character-wechat.js" || /\/bin\/st-character-wechat\.js$/u.test(binPath)) && normalized.length >= 4) {
      return ["st-character-wechat", normalizeString(normalized[2]), normalizeString(normalized[3])].filter(Boolean);
    }
    if (binBase) {
  ...
  if (executable === "st-character-wechat" || executable === "st-character-wechat.js") {
    return ["st-character-wechat", normalizeString(normalized[1]), normalizeString(normalized[2])].filter(Boolean);
  }
