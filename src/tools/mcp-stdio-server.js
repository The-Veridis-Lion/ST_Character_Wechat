    ...
    try {
      if (method === "initialize") {
        writeRpcResponse(id, {
          ...
          serverInfo: {
            name: "st-character-wechat-tools",
            version: "0.1.0",
          },
        }, reader.getMode());
        return;
...
function buildToolResources(toolCatalog) {
  ...
  const resources = [];
  resources.push({
    uri: "st-character-wechat://tools/index",
    name: "ST Character WeChat Tool Index",
    description: "Overview of ST Character WeChat project tools with schemas and usage notes.",
    mimeType: "text/markdown",
    text: buildToolIndexMarkdown(tools),
  });
  for (const tool of tools) {
    resources.push({
      uri: `st-character-wechat://tools/${tool.name}`,
...
function buildToolIndexMarkdown(tools) {
  const lines = [
    "# ST Character WeChat Project Tools",
    "",
    "These are ST Character WeChat project tools.",
    "",
  ];
  for (const tool of tools) {
