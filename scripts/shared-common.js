...
try {
  require("dotenv").config({ path: path.join(os.homedir(), ".st-character-wechat", ".env") });
} catch {
  // ignore
}

const rootDir = path.resolve(__dirname, "..");
const port = String(process.env.ST_CHARACTER_WECHAT_SHARED_PORT || "8765");
...
const sessionFile = process.env.ST_CHARACTER_WECHAT_SESSIONS_FILE || path.join(stateDir, "sessions.json");
...
async function ensureSharedAppServer() {
  if (process.env.ST_CHARACTER_WECHAT_RUNTIME && process.env.ST_CHARACTER_WECHAT_RUNTIME !== "codex") {
  ...
  const env = {
    ST_CHARACTER_WECHAT_STATE_DIR: stateDir,
    TIMELINE_FOR_AGENT_STATE_DIR: stateDir,
  };
  if (!process.env.TIMELINE_FOR_AGENT_CHROME_PATH) {
    env.TIMELINE_FOR_AGENT_CHROME_PATH =
      process.env.ST_CHARACTER_WECHAT_SCREENSHOT_CHROME_PATH
      || (process.platform === "darwin"
        ? "/Applications/Google Chrome.app/Contents/MacOS/Google Chrome"
        : "");
  }

  const command = process.env.ST_CHARACTER_WECHAT_CODEX_COMMAND || "codex";
  const mcpConfigArgs = buildCodexMcpConfigArgs(resolveCodexProjectToolMcpServerConfig({
    projectHome: process.env.ST_CHARACTER_WECHAT_HOME || rootDir,
  }));
...
function resolveBoundThread(workspaceRoot) {
  ...
  const runtimeId = normalizeText(process.env.ST_CHARACTER_WECHAT_RUNTIME || "codex");
