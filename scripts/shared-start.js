...
async function main() {
  ...
  if (existingBridgePid) {
    console.log(`shared st-character-wechat already running pid=${existingBridgePid}`);
    return;
  }
  ...
  if (isCodex) {
    childEnv.ST_CHARACTER_WECHAT_CODEX_ENDPOINT = listenUrl;
  }

  const child = spawn(process.execPath, ["./bin/st-character-wechat.js", "start", "--checkin"], {
    cwd: rootDir,
    env: childEnv,
    stdio: "inherit",
  });
