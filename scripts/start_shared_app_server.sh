...
ROOT_DIR="${0:A:h:h}"
PORT="${ST_CHARACTER_WECHAT_SHARED_PORT:-8765}"
LISTEN_URL="ws://127.0.0.1:${PORT}"
STATE_DIR="${ST_CHARACTER_WECHAT_STATE_DIR:-$HOME/.st-character-wechat}"
LOG_DIR="${STATE_DIR}/logs"
PID_FILE="${LOG_DIR}/shared-app-server.pid"
LOG_FILE="${LOG_DIR}/shared-app-server.log"
CODEX_COMMAND="${ST_CHARACTER_WECHAT_CODEX_COMMAND:-codex}"
...
mkdir -p "${LOG_DIR}"
export ST_CHARACTER_WECHAT_STATE_DIR="${STATE_DIR}"
export TIMELINE_FOR_AGENT_STATE_DIR="${STATE_DIR}"
if [[ -z "${TIMELINE_FOR_AGENT_CHROME_PATH:-}" ]]; then
  export TIMELINE_FOR_AGENT_CHROME_PATH="${ST_CHARACTER_WECHAT_SCREENSHOT_CHROME_PATH:-/Applications/Google Chrome.app/Contents/MacOS/Google Chrome}"
fi
export ST_CHARACTER_WECHAT_HOME="${ST_CHARACTER_WECHAT_HOME:-${ROOT_DIR}}"
  ...
  node -e '
    ...
    const args = helper.buildCodexMcpConfigArgs(
      helper.resolveCodexProjectToolMcpServerConfig({
        projectHome: process.env.ST_CHARACTER_WECHAT_HOME || process.argv[2],
      })
    );
